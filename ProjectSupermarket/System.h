...
private:
	 ...
	 void load_feed_list(const my_string& filename);
	 void load_pending_cashiers(const my_string& filename);
	 void load_categories(const my_string& filename);
	 ...
	 void save_feed_list(const my_string& filename);
	 void save_categories(const my_string& filename);
	 void save_pending_cashiers(const my_string& filename);

	 size_t get_transaction_id() const;
	 size_t delete_gift_card(const my_string& special_code);
	 ...
	 size_t convert_string_to_size_t(const my_string& str) const;
	 double round_to_decimal(double value, int decimal_places) const;
